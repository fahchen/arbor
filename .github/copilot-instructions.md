...
- **Codegen drift.** When a diff edits a `state do` block on an Arbor module, the TypeScript bundle at `priv/codegen/ts/arbor.ts` (or the consumer app's equivalent path under `config :arbor, :ts_codegen_output_path`) must be regenerated and committed. The `:arbor_ts` Mix compiler (`Mix.Tasks.Compile.ArborTs`) keeps it in sync on every `mix compile` once added to `compilers:`; `mix compile.arbor_ts --check` runs in `mix precommit` and returns a `Mix.Task.Compiler.Diagnostic` on drift. A CI failure there means the diff missed regenerating the bundle. Adding a new field-type AST shape (e.g. a non-trivial union, a new `stream(T)` site, a new cross-module reference) should also surface a `TypeRenderer` test in `test/arbor/codegen/type_script/type_renderer_test.exs`. Discovery is via per-module manifest entries written by `Arbor.Plugin.TypeScript`'s injected `@after_compile {Arbor.Codegen.TypeScript.Manifest, :__after_compile__}` callback (the callback uses `Macro.expand/2` against `env.aliases` to fully-qualify cross-module references before serializing). There is no persisted `:__arbor_ts__` attribute and no beam scan — diffs that drop the plugin from the chain will silently empty the codegen output.
