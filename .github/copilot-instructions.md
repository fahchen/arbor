...
- **Stream pending-ops invariants.** `Arbor.Stream` queues ops in `socket.private[:__arbor_pending_stream_ops__]` and the `:__arbor_stream_seen__` MapSet tracks which streams have a non-configure op queued in the current handler. `stream_configure/3` after a non-configure op for the same name must raise; pending ops must not survive across handlers (the page server flushes them once per command/handle_info/handle_async). Diffs that move the accumulator off `socket.private`, skip the configure-precedence check, or reuse pending ops across handlers are spec violations.
