...
- **Examples are not test deps.** `examples/<name>/` mini-apps depend on `arbor` via `path: "../.."`. Diffs that add them to the main project's `deps/0` or its test suite mistake documentation for runtime code.
