...
- **Stream pending-ops invariants.** `Arbor.Stream` queues deltas on a per-stream `%Arbor.LiveStream{}` held under `socket.assigns.__streams__[<name>]`. The runtime does **not** maintain an ordered `item_keys` list, decide upsert-vs-insert, or trim for `:limit` server-side — that's the client's job. `stream_configure/3` is a lifetime gate; raises if called after the stream's first init. The `Arbor.Hooks.PruneStreams` hook drains pending ops into `socket.private[:__arbor_drained_stream_ops__]` and prunes the struct via `Arbor.LiveStream.prune/1`; the page server collects from the accumulator. Diffs that re-introduce server-side `item_keys`, server-side trim, an `update_only` option, or that bypass the prune hook are spec violations.
