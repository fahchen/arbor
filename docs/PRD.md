...
| Render contract | `state(socket)` returns a value matching `state do`; `child(...)` placeholders are resolved bottom-up before validation/diffing. |
...
`state do` declares the value `state/1` returns. It is the single source of truth for typespecs, TypeScript codegen, and render-output validation. Field types include primitives, `list(...)`, `map()`, nested `Arbor.State` modules, references to other stores' `state()`, native Elixir typespec unions for variants, `stream(T)` markers (streams/lifecycle), and `AsyncResult.of(T)` markers (async/lifecycle).
...
`state(socket)` returns a value matching `state do`. Child stores are composed via `child(Module, id: ..., assign_key: value, ...)`, a render-time placeholder that the runtime resolves by mounting/updating the child node and substituting its render output:
...
A child's `id` must be a string; numeric ids must be `to_string/1`'d. Duplicate `(parent_path, module, id)` in one render output raises during reconcile. A child that disappears from `state/1` is unmounted; reappearance produces a fresh mount with no preserved assigns (BDR-0011).
...
```elixir
def handle_callback(:product_selected, %{id: id}, socket) do
...
All cross-cutting and per-node concerns use `attach_hook(socket, id, stage, fun)` (BDR-0004). There is no `middleware` macro. Stages: `:before_command`, `:after_command`, `:handle_async`, `:handle_info`, `:after_state`. Hook return: `{:cont, socket}`, `{:halt, socket}`, or `{:halt, reply, socket}` (only on `:before_command`). Mirrors `Phoenix.LiveView.attach_hook/4`. Each store maintains its own hook table; child-attached hooks see only that node's events. `detach_hook/3` is a silent no-op when the hook is absent.

Authors attach hooks inside `mount/1` for stable concerns and inside any handler for runtime-driven attachment. Built-in hooks (`Arbor.Hooks.ValidateCommandSchema` on `:before_command`, `Arbor.Hooks.ValidateState` on `:after_state`) are attached by the runtime's mount path; authors may detach or replace them (BDR-0007). Render-output validation is default-on in dev/test, telemetry-only opt-in for prod.
...
| State Resolver | Walks `state/1`'s return value and resolves `child(...)` placeholders bottom-up. |
| State Validator | Validates each store's resolved output via `Arbor.Hooks.ValidateState`. |
...
Error reply categories on the wire: `unknown_path | unknown_command | invalid_payload | unauthorized | hook_halt`. Handler-controlled business failures use `{:reply, %{ok: false, error: ...}, socket}` and arrive on the wire as `status: "ok"`.
...
| `state do ... end` | Declares the public output shape | Validated against `state/1` output |
...
| `handle_callback(name, payload, socket)` | Upward callback handler | Returns `{:noreply, socket}` |
| `handle_info(msg, socket)` | Server-side message handler | Returns `{:noreply, socket}` |
...
| `state(socket)` | Produce the public output shape | Required |
...
2. `state(socket)` returns a value structurally matching that shape, with `child(...)` placeholders permitted at any depth where another store's `state()` (or a structurally-equivalent map) is expected.
...
8. `state/1` must be free of observable side effects; the runtime may invoke it more than once per state change.
9. A `state/1` exception terminates the page runtime (let-it-crash, BDR-0003); reconnect mounts fresh.
...
A handler raise in `handle_command/3` or `state/1` terminates the page runtime (BDR-0003). A handler raise in `handle_async/3` is caught and recorded as `[:arbor, :async, :exception]`; the runtime continues (BDR-0020).

### Authorization

Authorization is a hook that returns `{:halt, {:error, ...}}` to deny commands. The runtime delivers an error reply with `category: "unauthorized"` (BDR-0008). There is no silent-no-op downgrade.
...
defmodule MyApp.Stores.ProductPageStore do
  ...
  def handle_callback(:filters_changed, filters, socket) do
  ...
  def state(socket) do
    %{
      ...
      filters: child(FilterStore,
        ...
        on_change: fn payload -> handle_callback(:filters_changed, payload, socket) end
      ),
      products:
...
| M3: Command pipeline | Path-based routing, payload schema validation hook, attach_hook/detach_hook, authorization hook, `handle_command/3`, `handle_callback/3`, transport reply contract, error category enum, system command namespace. | High | Weeks 5–6 |
...
Persistence is **not** an Arbor primitive (recorded in `spec/backlog.md`). Applications load snapshots inside their own `mount/1` body and save via `attach_hook` on `:after_command`. Valid hook stages: `:before_command`, `:after_command`, `:handle_async`, `:handle_info`, `:after_state`.
...
- `state(socket)` returns a value matching `state do`, with `child(...)` placeholders permitted; the resolver substitutes them bottom-up.
...
- Transport reply uses Phoenix Channel ref reply (BDR-0001). Outcome ordering is reply → patch → effects (BDR-0009). Auth failure is hard error (BDR-0008). Wire error categories: `unknown_path | unknown_command | invalid_payload | unauthorized | hook_halt`.
- `attach_hook/4`, `detach_hook/3` work at any node (root or child) for stages `:before_command`, `:after_command`, `:handle_async`, `:handle_info`, `:after_state` (BDR-0004).
