...
- Compile-time lint: `Arbor.Async.Macros` wraps `assign_async`/`start_async` with `IO.warn` on socket capture inside the task fun. The walk only inspects literal `fn …` / `&…` AST so `start_async(socket, :foo, build_fn(socket))` (where `socket` flows through a helper) does not false-warn. `cancel_async` is wrapped only as a delegate — no capture check. Auto-imported by `Arbor.Store.__using__/1`. `stream_async` is intentionally not wrapped at the macro layer (the `state do` DSL exposes a same-named macro for declaring async-wrapped stream fields and Elixir cannot disambiguate same-name imports by argument count) — call the runtime form via `Arbor.Async.stream_async/3,4`
...
- Codegen: `mix arbor.codegen.ts` writes one `.ts` file per Arbor `state do` module to `priv/codegen/ts/` (configurable via `config :arbor, :ts_codegen_output_path`). Output files include the `AsyncResult<T>` generic, a `<Module>State` type, and (for stores) a `<Module>Commands` type. The generator scans the *current Mix project's* `:application.get_key/2` modules and skips `test/support/` sources; arbor itself emits no files. `mix arbor.codegen.ts --check` is wired into `mix precommit` and exits non-zero on drift
- The `Arbor.Plugin.TypeScript` plugin (auto-applied by `Arbor.DSL.State.state/1`) stamps a persisted `:__arbor_ts__` module attribute the codegen task uses to filter eligible modules. Removing the plugin from the chain breaks codegen
- Examples live under `examples/<name>/` as standalone mini-apps that depend on `arbor` via `path: "../.."`. They are documentation, not test deps — do not add them to the main project's `deps/0`
- Reference Phoenix Channel adapter (`Arbor.Transport.Channel`) emits `[:arbor, :channel, :join]` and `[:arbor, :channel, :terminate]`. Adapter `terminate/2` `GenServer.stop`s the linked page server with `:shutdown` so the runtime's own `terminate/2` runs with the original reason. Reconnect is recovery (BDR-0015): each join builds a fresh page server with `version: 1`
