...
- Compile-time lint: `Arbor.Async.Macros` wraps `assign_async`/`start_async`/`cancel_async` with `IO.warn` on socket capture inside the task fun. Auto-imported by `Arbor.Store.__using__/1`. `stream_async` is intentionally not wrapped (the `state do` DSL exposes a same-named macro for declaring async-wrapped stream fields and Elixir cannot disambiguate same-name imports by argument count) — call the runtime form via `Arbor.Async.stream_async/3,4`
