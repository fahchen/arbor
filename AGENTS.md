...
- Codegen: `mix arbor.codegen.ts` writes a single TypeScript bundle (`priv/codegen/ts/arbor.ts` by default, configurable via `config :arbor, :ts_codegen_output_path`) whose nested `export namespace <Segment>` blocks mirror the Elixir module tree. Type names equal each module's last alias segment; Stores with declared commands emit a sibling `export namespace <LastSegment> { export type Commands = ... }`. Cross-module references render the full Elixir alias path (codegen pre-resolves user `alias`'s by walking the host module's parent namespaces). The generator scans the *current Mix project's* `:application.get_key/2` modules and skips `test/support/` sources; arbor itself emits no bundle. `mix arbor.codegen.ts --check` is wired into `mix precommit` and exits non-zero on drift
...
- Application-level persistence pattern lives in `docs/persistence-pattern.md`: load inside `mount/1`, save via `attach_hook(:persist, :after_command, fun)`. Arbor ships no `Arbor.Persistence` module per `spec/backlog.md`
