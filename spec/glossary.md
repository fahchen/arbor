...
| Memoization | Per-key change tracking. The `assign/3` macro records each mutated key in `socket.assigns.__changed__` (skipping no-op writes via `===`). A child whose consumed-key set does not intersect `__changed__` skips its `update/2` and `to_state/1` invocations and reuses the previously resolved output. The runtime clears `__changed__` to `%{}` after each render cycle. Mirrors `Phoenix.LiveView` / `Phoenix.Component.assign/3`. |
