...
| Hook | A function attached at a specific lifecycle stage on a store node via `attach_hook/4`; analogous to `Phoenix.LiveView.attach_hook/4`. Stages: `:before_command`, `:after_command`, `:handle_async`, `:handle_info`, `:after_state`. The sole extension primitive — Arbor has no `middleware` macro (see BDR-0004). |
...
| Render-output validation hook | A built-in hook (`Arbor.Hooks.ValidateState`) attached on `:after_state` by the runtime's mount path. Validates each store's resolved render output against its `state do` declaration. Default-on in dev/test; telemetry-only opt-in for prod. |
...
| Memoization | A child whose `socket.assigns` map is reference-equal across render cycles skips its `update/2` and `state/1` invocations and reuses the previously resolved output. |
...
| `reload_stream/2` | Store callback returning `{:ok, [item]}` for stream recovery. Invoked only when the application explicitly calls `socket \|> reload_stream(name)`. The runtime never auto-invokes it. |
...
| `stream_async/4` | Composite async-and-stream API: spawns a task, holds an `AsyncResult` in `socket.assigns.<name>` for client-visible loading state, and on success seeds the named stream slot with the returned items. The AsyncResult's `result` field is `true` on success — items live in the stream, not the AsyncResult. |
| `invoke(socket, callback_name, payload)` | Child-side helper that calls the parent-supplied function attr stored at `socket.assigns.<callback_name>` with `payload` as the single argument. The function's return value replaces `socket` (the parent's `handle_callback/3` returns a new socket that flows back as the child's updated socket for the rest of the handler). |
| `handle_callback(name, payload, socket)` | Parent-side store callback that receives upward callback invocations from a child. Returns `{:noreply, socket}`. Wired up by passing the parent's `handle_callback`-bound function to the child via `child(Module, id: ..., on_x: fn payload -> handle_callback(:x, payload, socket) end)` (or via a runtime helper that produces the same closure). |
