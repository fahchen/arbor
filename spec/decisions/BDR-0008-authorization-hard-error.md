...
A silent-ok downgrade for unauthorized commands seems convenient ("UI shouldn't expose denied buttons anyway") but is hostile to auditing and debugging — denied actions disappear from logs and from client error handling. Hard error replies make denials observable on both sides without requiring extra telemetry plumbing. If specific commands genuinely should silently succeed for unauthorized actors (e.g., feature-disabled placeholders), the application can express that explicitly via a `:before_command` hook returning `{:halt, %{ok: true}, socket}`, which is auditable in code and in telemetry under category `hook_halt` if desired.
