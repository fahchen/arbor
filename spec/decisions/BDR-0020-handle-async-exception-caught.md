...
BDR-0003 established let-it-crash semantics for command handlers and `render/1`: a raise terminates the page runtime; the supervisor restarts; reconnect re-mounts from snapshot. This is appropriate for synchronous user-driven flows where the client can immediately retry or perceive the disruption.
