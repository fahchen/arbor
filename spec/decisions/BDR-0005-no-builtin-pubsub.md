...
Earlier PRD drafts proposed an Musubi-owned PubSub layer with a `subscribe fn socket -> [...] end` declaration block, a `broadcast/4` helper on `socket`, and a `handle_broadcast/3` callback. The motivation was same-user cross-page sync. The design adds an Musubi-specific abstraction above `Phoenix.PubSub` that has to be learned, documented, and maintained.
...
- It introduces an Musubi-specific layer over `Phoenix.PubSub` that does not gain functionality.
...
Option C was rejected for the same reasons as B but with a smaller surface — still adds an Musubi-specific subscribe macro that users must learn instead of just calling `Phoenix.PubSub.subscribe/2`.
