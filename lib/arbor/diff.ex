defmodule Arbor.Diff do
  ...
  Arbor only emits `add`, `remove`, and `replace` ops. `move`, `copy`, and
  ...
  ## Examples
      ...
      iex> Arbor.Diff.diff(%{"a" => 1}, %{"a" => 2})
      [%{op: "replace", path: "/a", value: 2}]

      iex> Arbor.Diff.diff(%{"a/b" => 1}, %{"a/b" => 2})
      [%{op: "replace", path: "/a~1b", value: 2}]

      iex> Arbor.Diff.diff(%{"a" => 1}, %{"a" => 1})
      []
  """

  alias Arbor.Telemetry

  @typedoc "RFC 6902 op kind that Arbor emits — `move`/`copy`/`test` are excluded."
  ...
  @doc """
  Returns the Arbor-allowed RFC 6902 ops describing the change from `previous`
  to `current`.

  Both inputs must be wire-form terms (string keys, plain maps, atoms encoded
  as strings) — pass values that have already gone through `Arbor.Wire.to_wire/1`
  ...
  ## Examples
      ...
      iex> Arbor.Diff.diff(%{"title" => "Inbox"}, %{"title" => "Outbox"})
      [%{op: "replace", path: "/title", value: "Outbox"}]

      iex> Arbor.Diff.diff(%{"items" => [1, 2, 3]}, %{"items" => [1, 9, 3]})
  ...
  defp sanity_check!(value) when is_function(value) do
    raise ArgumentError,
          "Arbor.Diff received a function value: #{inspect(value)}. " <>
