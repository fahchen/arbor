defmodule Musubi.AsyncSupervisor do
  @moduledoc """
  `Task.Supervisor` that hosts every async task spawned by
  `Musubi.Async.assign_async/3,4`, `Musubi.Async.start_async/3,4`, and
  `Musubi.Async.stream_async/3,4`.

  Started automatically by `Musubi.Application`. Applications that want to
  ...
  server (and is converted to `Musubi.AsyncResult.failed(prior, {:exit, reason})`)
  rather than crashing the runtime.
  """

  @doc "Returns the child spec used by `Musubi.Application` to start the supervisor."
